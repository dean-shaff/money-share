...
const { User, Rotation, MemberRotation } = require("./../models")
...
module.exports = {
  getUserRotations: async function (request, h) {
    ...
    const rotations = await Rotation.findAll({
      ...
      order: [[{model: User, as: 'members'}, MemberRotation, 'rotationIndex', 'DESC']]
    })
    return rotations
  },
  ...
  update: async function (request, h) {
    ...
    const updateKeys = ['name', 'cycleAmount', 'cycleDuration', 'started', 'dateStarted']

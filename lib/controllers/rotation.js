...
module.exports = {
  getUserMemberRotations: async function (request, h) {
    ...
    let includeObj = Object.assign({ where: { 'id': request.params.userId }}, memberCycleNoteInclude)
    ...
    return await Rotation.findAll({
      ...
      include: [memberCycleNoteInclude, managerInclude],
  ...
  update: async function (request, h) {
    ...
    let rotation = await Rotation.findOne(options)
    console.log(`controllers.rotation.update: (before manager/member updates) rotation=${JSON.stringify(rotation, null, 2)}`)
    ...
    rotation = await Rotation.findOne(options)
    console.log(`controllers.rotation.update: (after manager/member updates) rotation=${JSON.stringify(rotation, null, 2)}`)
    return rotation
  },

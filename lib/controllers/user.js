...
module.exports = {
  ...
  update: async function (request, h) {
    ...
    const keys = ['name', 'username', 'phone', 'email', 'autoCreated']
    const user = await User.findByPk(id)
    keys.forEach(key => {
      if (request.payload[key] !== undefined) {
        console.log(`controllers.user.update: updating ${key} to ${request.payload[key]}`)
  ...
  get: async function (request, h) {
    ...
    if (id === undefined || id === '') {
      if (Object.keys(request.query).length === 0) {
        ...
        const where = Object.entries(request.query).map(([key, value]) => ({[key]: value}))

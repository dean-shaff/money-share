...
const getHome = function (request, h) {
  console.log(`getHome: publicDir=${publicDir}`)
...
const forgotPasswordHandler = async (request, h) => {
  ...
  if (! user) {
    throw Boom.badRequest("That user doesn't exist")
  }
...
const resetPasswordHandler = async (request, h) => {
  ...
  console.log(`resetPasswordHandler: token=${token}`)
  const user = await User.findOne({
  ...
  console.log(`resetPasswordHandler: expires=${expires}`)
  console.log(`resetPasswordHandler: now=${now}`)
